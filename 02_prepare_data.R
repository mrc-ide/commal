...
dhs_sma <- dhs_data_raw  %>%
  ...
  mutate(
    ...
    #sma2 = ifelse(anemia_level == "severe" & microscopy == "positive" & fever == "yes", 1, 0)
  ...
  dplyr::select(iso, country, cluster, survey_year, year, pfpr, microscopy, hb, sma, sma_fever, chronic_anaemia) %>%
